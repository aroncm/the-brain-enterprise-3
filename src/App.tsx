...
// for now; per-pitcher norm when backend ships them). Used by individual
// factor cards (Swinging Strike Rate, Strike Rate, Hard Contact, etc.)
// to show "how is the pitcher doing on this dimension right now relative
// to expectation" — distinct from peak-to-date operational concern.
//
//   direction "high-good": higher actual = better (whiff, strike rate, etc.)
//   direction "low-good":  lower actual = better (hard contact, zone miss)
//
// Returns: { scaledPercent (bar fill 0..1), role (good/warn/bad), label }.
// The bar fill represents *concern level* — higher fill = worse vs reference.
type FactorVsReference = {
  scaledPercent: number;
  role: "good" | "warn" | "bad";
  label: string;
};

function factorVsLeague(
  ...
  // gold = in between (off his norm but not yet below league). Without a
  // baseline, fall back to the league-only bands. A ±graceFraction tolerance
  // keeps borderline values from flickering between bands.
  const better = (x: number, ref: number) =>
    direction === "high-good" ? x >= ref * (1 - graceFraction) : x <= ref * (1 + graceFraction);
  const base = num(pitcherNorm);
  let role: "good" | "warn" | "bad";
  let label: string;
  if (base != null) {
    if (better(a, base)) {
      role = "good";
      label = "At or above his baseline.";
    } else if (!better(a, reference)) {
      role = "bad";
      label = "Below league avg.";
    } else {
      role = "warn";
      label = "Below his baseline, still at/above league.";
    }
  } else {
    if (direction === "high-good") {
      role = delta > graceFraction ? "good" : delta >= -graceFraction ? "warn" : "bad";
    } else {
      role = delta < -graceFraction ? "good" : delta <= graceFraction ? "warn" : "bad";
    }
    label =
      role === "good" ? "Better than league avg." : role === "warn" ? "Near league avg." : "Worse than league avg.";
  }
  return { scaledPercent, role, label };
}

function factorVsLeagueTone(
  cmp: FactorVsReference | null,
): "good" | "gold" | "bad" | "neutral" {
...
// Step 3 / Phase F — dual tick helper for diagnostic factor cards. Always
// returns the league tick at the 0.1 at-reference position; appends a
// baseline tick when the backend has surfaced a pitcher_norm value with
// enough sample. When the baseline tick lands within 0.10 of the league
// tick the two ::after labels would render on top of each other ("CAREERUE"
// from the prior naming) — so we suppress the league LABEL only (keeping
// the tick element via an empty label). Colors still distinguish the two
// ticks (white = league, teal = baseline) and the detail copy below the
// bar names both numbers explicitly.
function dualTicks(
...
// One-line plain-English pitcher baseline comparison, e.g.
// "Pitcher baseline 13% (above league avg 11%)." Returns "" when norm is
// unavailable so callers can append unconditionally.
function pitcherBaselinePhrase(
  pitcherNorm: number | null | undefined,
  leagueRef: number,
  ...
  if (n > leagueRef * 1.02) return ` Pitcher baseline ${formattedNorm} (above league avg ${formattedLeague}).`;
  if (n < leagueRef * 0.98) return ` Pitcher baseline ${formattedNorm} (below league avg ${formattedLeague}).`;
  return ` Pitcher baseline ${formattedNorm} (matches league avg).`;
}
...
// finite score for the requested key.
function componentPeak(
  ...
  for (let i = 0; i < cap; i++) {
    const scores =
      (entries[i]?.snapshot?.starter_state as { normalized_component_scores?: Record<string, number | null> } | undefined)
...
function weightedSumPeak(
  ...
  for (let i = 0; i < cap; i++) {
    const wc =
      (entries[i]?.snapshot?.starter_state as { normalized_weighted_components?: Record<string, number | null> } | undefined)
        ?.normalized_weighted_components;
    if (!wc) continue;
    let sum = 0;
    let any = false;
    for (const k of keys) {
...
// Cumulative max of starter_state.normalized_degradation_score (the
// pitcher-only composite, 0..1). Distinct from headlinePeak because the
// pitcher-only card explicitly does NOT include relief / decision context.
function pitcherOnlyPeak(
  entries: PitchingReplayEntry[],
  selectedIndex: number,
): number | undefined {
  let peak: number | undefined;
  const cap = Math.min(selectedIndex + 1, entries.length);
  for (let i = 0; i < cap; i++) {
    const v = (entries[i]?.snapshot?.starter_state as { normalized_degradation_score?: number | null } | undefined)
...
function scaledConcern(value: number | null | undefined, scale = 1): number | null {
  if (value == null || !Number.isFinite(value) || scale <= 0) return null;
  return clamp(value / scale);
}

const SIGNAL_PRESSURE_THRESHOLDS = {
  watch: 0.15,
  prep: 0.42,
  pullNow: 0.69,
};

function decisionPressureThresholds(entry: PitchingReplayEntry | null): typeof SIGNAL_PRESSURE_THRESHOLDS {
  const thresholds = entry?.recommendation.decision_pressure_thresholds;
  return {
    watch: num(thresholds?.watch) ?? SIGNAL_PRESSURE_THRESHOLDS.watch,
    prep: num(thresholds?.prep) ?? SIGNAL_PRESSURE_THRESHOLDS.prep,
    pullNow: num(thresholds?.pullNow) ?? num(thresholds?.pull_now) ?? SIGNAL_PRESSURE_THRESHOLDS.pullNow,
  };
}

function signalBand(status: string, thresholds = SIGNAL_PRESSURE_THRESHOLDS): { min: number; max: number } {
  const label = statusLabel(status);
  if (label === "PULL NOW" || label === "DISTRESS") return { min: thresholds.pullNow, max: 1 };
  if (label === "PREP") return { min: thresholds.prep, max: thresholds.pullNow - 0.01 };
  if (label === "WATCH") return { min: thresholds.watch, max: thresholds.prep - 0.01 };
  return { min: 0, max: thresholds.watch - 0.01 };
}

function recommendationPressure(entry: PitchingReplayEntry | null): number | null {
  if (!entry) return null;
  return num(entry.recommendation.decision_pressure_score);
}

function signalAlignedDegradation(entry: PitchingReplayEntry | null, displayedStatus: string): number | null {
  if (!entry) return null;
  const band = signalBand(displayedStatus, decisionPressureThresholds(entry));
  const raw = recommendationPressure(entry);
  if (raw == null) return band.min;
  return clamp(raw, band.min, band.max);
}

function cumulativeSignalAlignedDegradation(
  entries: PitchingReplayEntry[],
  statuses: string[],
  selectedIndex: number,
): number | null {
  if (entries.length === 0) return null;
  const cappedLength = Math.min(selectedIndex + 1, entries.length);
  const values = entries
    .slice(0, cappedLength)
    .map((entry, index) => signalAlignedDegradation(entry, statuses[index] ?? entry.recommendation.status))
    .filter((value): value is number => value != null && Number.isFinite(value));
  return values.length ? Math.max(...values) : null;
}

function pitcherOnlyDegradationConcern(entry: PitchingReplayEntry | null): number | null {
  if (!entry) return null;
  const state = replayState(entry);
  return scaledConcern(
    num(entry.recommendation.independent_degradation_score) ??
      num(state.degradation_score) ??
      num(state.normalized_degradation_score),
    1,
  );
}
...
function currentStuffConcern(entry: PitchingReplayEntry | null): number | null {
  ...
  const state = replayState(entry);
  const values = [
    velocityDrop(entry) == null ? null : scaledConcern(Math.abs(velocityDrop(entry) ?? 0), 4),
    num(state.spin_slope_5) == null ? null : scaledConcern(Math.abs(num(state.spin_slope_5) ?? 0), 250),
    scaledConcern(Math.abs(num(state.pitch_mix_drift_10) ?? NaN), 1),
  ].filter((value): value is number => value != null && Number.isFinite(value));
  return values.length ? Math.max(...values) : null;
}
...
function currentCommandConcern(entry: PitchingReplayEntry | null): number | null {
  ...
  const state = replayState(entry);
  const values = [
    scaledConcern(num(state.zone_miss_distance_10), 0.8),
    scaledConcern(num(state.location_dispersion_10), 1.4),
    scaledConcern(num(state.ball_rate_10), 1),
  ].filter((value): value is number => value != null && Number.isFinite(value));
  return values.length ? Math.max(...values) : null;
}
...
function currentWorkloadConcern(entry: PitchingReplayEntry | null): number | null {
  if (!entry) return null;
  const state = replayState(entry);
  const inning = num(state.inning_decay_factor);
  const tto = num(state.tto_decay_factor);
  if (inning == null && tto == null) return null;
  return scaledConcern((inning ?? 0) + (tto ?? 0), 3);
}

function currentLeverageConcern(entry: PitchingReplayEntry | null): number | null {
  if (!entry) return null;
  return scaledConcern(num(entry.snapshot.leverage_index), 3);
}
...
function factorMeterDetail(detail: string): string {
  return `${detail} Meter tracks peak pressure this appearance.`;
}
...
function candidatesForEntry(entry: PitchingReplayEntry | null): PitchingRelieverCandidate[] {
  if (!entry) return [];
  const candidates = [
    ...(entry.top_candidates ?? []),
    ...(entry.snapshot.reliever_candidates ?? []),
  ];
  const byKey = new Map<string, PitchingRelieverCandidate>();
  for (const candidate of candidates) {
    const key = String(candidate.player_id || candidate.player_name || "").trim();
...
function reliefCandidatesThroughPitch(
  entries: PitchingReplayEntry[],
  selectedIndex: number,
  selected: PitchingReplayEntry | null,
): PitchingRelieverCandidate[] {
  const byKey = new Map<string, PitchingRelieverCandidate>();
  const addCandidate = (candidate: PitchingRelieverCandidate) => {
    const key = String(candidate.player_id || candidate.player_name || "").trim();
    if (!key || byKey.has(key)) return;
    byKey.set(key, candidate);
  };
  candidatesForEntry(selected).forEach(addCandidate);
  const capped = entries.slice(0, Math.min(selectedIndex + 1, entries.length)).reverse();
  for (const entry of capped) {
    candidatesForEntry(entry).forEach(addCandidate);
  }
    ...
    <div className={cardClassFinal}>
      ...
      {percent != null ? (
        <div className={trackClass} aria-hidden="true">
          {benchmarks
            ? benchmarks.map((bm, i) => {
                const clampedBm = clamp(bm.percent);
                // Phase F.3 — edge anchor: labels near 0/100 clip when the
                // ::after element uses translateX(-50%) centering. Tag near-
                // edge ticks so CSS can anchor the label to the bar edge
                // instead of overflowing.
                const edge = clampedBm < 0.08 ? "left" : clampedBm > 0.92 ? "right" : undefined;
                return (
                  <span
                    key={`${bm.label ?? i}-${bm.percent}`}
                    className="gauge-benchmark gauge-benchmark--labeled"
                    style={{ left: `${Math.round(clampedBm * 100)}%` }}
                    data-label={bm.label ?? ""}
                    data-edge={edge}
                  />
                );
...
function GameAudit({
  ...
  const rssCommandPeak = selectedIsReliever
    ...
    : undefined;
  // Phase X.9 — if user is on Relief Edge and switches to a reliever
  // (which hides that tab), fall back to Signal Summary. (The Comparison
  // tab was removed entirely in Phase II per Collin's feedback.)
  useEffect(() => {
    if (selectedIsReliever && rightTab === "relief") {
      setRightTab("signal");
    }
  ...
  // Signal Summary cards — composites from normalized_weighted_components
  // (already in concern-units, summed per entry, peak across entries).
  const stuffPressure = weightedSumPeak(entries, selectedIndex, [
    "velocity",
    "pitch_type_velocity",
    "spin",
    "pitch_type_spin",
  ]);
  const commandPressure = weightedSumPeak(entries, selectedIndex, [
    "command",
    "zone_control",
    "whiff",
    "contact",
  ]);
  const decayPressure = weightedSumPeak(entries, selectedIndex, [
    "inning_context",
    "tto_context",
    "pitch_mix",
  ]);
  // Per-factor peaks driven by backend normalized_component_scores.
            ...
            <div className="replay-layout">
              ...
              <aside className="pitch-window-summary pitch-window-summary--compact" style={{ borderTop: `3px solid ${accents.accent}` }}>
                ...
                <div className="pws-trend-row">
                  {/* Phase II — 0 mph / 0 rpm are impossible readings; they
                    * appear when the pitch-type window has no usable data
                    * (e.g. no tracked pitches of this type yet). Render
                    * "n/a" instead of a misleading zero. */}
                  {(() => {
                    const veloVal = trendNumber(currentVelocityTrend?.bucket ?? null, "mean_last5") ?? selected.snapshot.release_speed ?? selectedState?.velo_mean_5;
                    const spinVal = trendNumber(currentSpinTrend?.bucket ?? null, "mean_last5") ?? selectedState?.spin_mean_5;
                    const veloLabel = veloVal == null || !Number.isFinite(veloVal) || veloVal <= 0 ? "n/a" : `${fmtNumber(veloVal, 1)} mph`;
                    const spinLabel = spinVal == null || !Number.isFinite(spinVal) || spinVal <= 0 ? "n/a" : `${fmtNumber(spinVal, 0)} rpm`;
                    return (
                      <>
                        <TrendSparkline
                          label={`${matchedVelocityPitchLabel} Velocity`}
                          value={veloLabel}
                          detail={veloLabel === "n/a" ? "No tracked pitches of this type in the window yet." : `Baseline ${fmtNumber(trendNumber(currentVelocityTrend?.bucket ?? null, "baseline") ?? selectedState?.seasonal_velo_baseline, 1)} · drop ${fmtNumber(trendNumber(currentVelocityTrend?.bucket ?? null, "drop"), 1)}`}
                          points={currentPitchVeloPoints}
                        />
                        <TrendSparkline
                          label={`${matchedSpinPitchLabel} Spin`}
                          value={spinLabel}
                          detail={spinLabel === "n/a" ? "No tracked pitches of this type in the window yet." : `Baseline ${fmtNumber(trendNumber(currentSpinTrend?.bucket ?? null, "baseline") ?? selectedState?.seasonal_spin_baseline, 0)} · trend ${fmtSigned(trendNumber(currentSpinTrend?.bucket ?? null, "slope") ?? selectedState?.spin_slope_5, 0)}`}
              ...
              {/* Phase S.3 — right panel is now tabbed. Three tabs since
                * Phase II removed Comparison (Collin: percentiles read as
                * "random numbers" without context):
                * Signal Summary | Model Detail | Relief Edge.
              ...
              <aside className="model-synthesis-card model-synthesis-card--tabbed">
                <div className="replay-tabs" role="tablist" aria-label="Replay detail tabs">
                  ...
                  {/* Phase X.8/X.9 — Relief Edge tab is hidden for
                    * relievers. Reliever-variant right panel reduces to
                    * Signal Summary + Model Detail only. */}
                  {!selectedIsReliever ? (
                  ...
                  <div className="replay-tab-panel">
                    ...
                    <div className="model-detail-grid">
                      <div className="model-detail-col">
                        {(() => {
                          // Phase II — this card previously displayed the CALLED-strike
                          // share (strike_rate_10 is built from called-strike flags
                          // backend-side) against the 0.63 overall-strike league
                          // benchmark — which is why it always read ~20% and red.
                          // True strike share = 1 − ball rate (everything not taken
                          // for a ball: swings, fouls, contact, called strikes). The
                ...
                {/* Phase II — the Comparison tab (S.7 vs League / vs Career
                  * percentile cards) was removed per Collin's feedback: the
                  * percentile framing read as "random numbers" without
                  * actionable context. The underlying fields
                  * (empirical_degradation_percentile et al.) remain in the
                  * payload if a better framing earns the space back. */}
              </aside>
            </div>
          ...
          <article ref={aosRef} className={`panel counterfactual-panel${outcomeOpen ? "" : " panel--collapsed"}`}>
            ...
            <div className="counterfactual-grid">
              ...
              <div className="outcome-card">
                ...
                {/* Phase II — the "if left in" counterfactual returns (it was
                  * removed in FF.2 as an unlabeled orphan). It reads as part
                  * of the actual-outcome narrative: only rendered when the
                  * manager held the starter past the Pull Now signal and the
                  * post-signal peak is at least the at-signal value. */}
                {(() => {
                  if (starterPullIndex < 0 || starterPullIndex >= starterEntries.length - 1) return null;
                  let atSignal: number | undefined;
                  for (let i = 0; i <= starterPullIndex; i++) {
                    const s = replayState(starterEntries[i]) as { enhanced_degradation_score?: number | null } | null;
                    const raw = num(s?.enhanced_degradation_score);
                    if (raw == null) continue;
                    const scaled = Math.max(0, Math.min(1, raw / 3));
                    atSignal = atSignal === undefined ? scaled : Math.max(atSignal, scaled);
                  }
                  ...
                  if (atSignal == null || leftInPeak == null) return null;
                  const atSignalPct = Math.round(atSignal * 100);
                  const leftInPct = Math.round(leftInPeak * 100);
                  if (leftInPct < atSignalPct) return null;
                  return (
                    <p>
                      Over the pitches after the signal, his Adjusted Degradation peaked at <b>{leftInPct}%</b> — vs <b>{atSignalPct}%</b> at the signal.
                    </p>
                  );
                })()}
              </div>
